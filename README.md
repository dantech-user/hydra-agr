...
- preferências de notificações persistentes, segurança da conta, apoio voluntário e documentos completos no Perfil;
- clima real da cidade cadastrada, com temperatura, sensação, umidade, chuva, vento, mínima e máxima via Open-Meteo;
...
- **Drone Pastor:** o modelo de dados e a central de monitoramento existem; a conexão com hardware/API compatíveis está prevista para a próxima atualização e nenhuma missão real é iniciada nesta versão;
...
```text
src/
  components/          UI reutilizável, controlador global de modais e toasts
  features/
  ...
  services/            Supabase, mídia, NFC e clima regional
...
### Sistema global de modais e formulários

`src/components/modal-system.ts` mantém uma pilha única de sobreposições. Enquanto existir qualquer modal ou bottom sheet aberto, a navegação inferior desliza totalmente para fora da viewport e deixa de reservar espaço. A barra só retorna depois que a última sobreposição for fechada, inclusive quando há modais encadeados.

Os componentes compartilhados em `src/components/ui.tsx` fornecem:

- overlay com escurecimento e desfoque;
- altura dinâmica baseada em `100dvh`, safe areas e teclado mobile;
- cabeçalho fixo, conteúdo com scroll interno e ações sempre acessíveis;
- botões com bloqueio de envio duplicado, spinner e mensagem de carregamento;
- erros dentro do formulário sem apagar os valores digitados;
- confirmação obrigatória para ações destrutivas;
- toast de sucesso ou falha acima de todas as camadas.

A demonstração navegável e independente está em `hydra-agro-modal-preview.html`. Abra esse arquivo diretamente no navegador para testar registro de água, cadastro de animal, criação de setor, exclusão e Hydra Agro+ antes de iniciar o app completo.
...
O primeiro endereço abre a recuperação no APK. O segundo abre a mesma tela na versão web; substitua `SEU-DOMINIO` pelo domínio publicado. Durante um teste no Codespaces, adicione também a URL HTTPS encaminhada da porta 5173 seguida de `/auth/recovery`. Isso permite que “Esqueci minha senha” volte ao aplicativo e abra a criação de nova senha sem expor nenhuma chave privada.
...
A tela inicial consulta automaticamente o clima do município cadastrado usando a API pública do Open-Meteo. A integração:

- localiza somente municípios brasileiros e prioriza o resultado da Bahia;
- exibe condições reais, sem números de demonstração;
- mantém cache local por 20 minutos para evitar chamadas repetidas;
- usa por até seis horas o último dado salvo quando o serviço fica temporariamente indisponível, identificando-o como desatualizado;
- não envia nome, e-mail, animais ou registros rurais ao serviço meteorológico;
- não exige chave no frontend.

O clima depende de internet para atualização. Antes de uso comercial em escala, revise os termos e o plano de uso vigentes do provedor Open-Meteo.
...
./gradlew bundleRelease \
  -PHYDRA_VERSION_CODE=5 \
  -PHYDRA_VERSION_NAME=1.2.2 \
...
A suíte automatizada verifica cobertura regional, validação de autenticação, isolamento estrutural de contas, ausência de concessão administrativa no cliente, proteção do Hydra Agro+, contratos dos botões, pilha global de modais, remoção da bottom bar, preservação do formulário após erro, RLS/owner no banco e empacotamento Capacitor sem URL remota. O build TypeScript/Vite e o `cap sync android` também fazem parte da validação de entrega.
