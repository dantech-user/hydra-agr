    ...
    <div className="screen page-enter extra-screen property-screen">
      ...
      <Modal open={editOpen} onClose={() => { setEditOpen(false); setError(""); }} eyebrow="EDIÇÃO" title="Dados da propriedade" wide dismissible={!saving}>
        <form className="modal-form" onSubmit={save}>
          ...
          <div className="municipality-field-grid"><Field label="Município"><MunicipalityPicker value={draft.municipality} onChange={(municipality) => { setDraft({ ...draft, municipality }); setError(""); }} /></Field><Field label="Estado"><div className="state-readonly"><span>BA</span><strong>Bahia</strong></div></Field></div>
          ...
          {error && <p className="form-error" role="alert">{error}</p>}<div className="modal-action-row"><button className="secondary-button" type="button" onClick={() => setEditOpen(false)} disabled={saving}>Cancelar</button><LoadingButton className="primary-button" type="submit" loading={saving} loadingLabel="Salvando propriedade...">Confirmar alterações</LoadingButton></div>
        </form>
      </Modal>
    </div>
  );
