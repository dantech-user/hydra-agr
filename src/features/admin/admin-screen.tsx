...
import { EmptyState, Field, Modal, ScreenHeader, Toggle } from "../../components/ui";
...
type Tab = "overview" | "users" | "announcements" | "links" | "moderation";
  ...
  return (
    <div className="screen page-enter extra-screen admin-screen">
      ...
      <div className="admin-tabs" role="tablist" aria-label="Seções administrativas">
        {([
          ["overview", "Visão geral"], ["users", "Usuários"], ["announcements", "Avisos"], ["links", "Links"], ["moderation", "Moderação"],
      ...
      <Modal open={Boolean(selectedUser)} onClose={() => setSelectedUser(null)} eyebrow="GESTÃO DE USUÁRIO" title={selectedUser?.name || "Usuário"} wide>
        {selectedUser && <div className="admin-user-detail">
          ...
          <Field label="Permissão">
            <select value={selectedUser.role} disabled={account.role !== "owner" || selectedUser.role === "owner" || selectedUser.id === account.id} onChange={(event) => void run(() => setUserRole(selectedUser.id, event.target.value as "user" | "moderator" | "admin"), "Permissão atualizada.")}>
          ...
          <button className={selectedUser.bannedAt ? "secondary-button full" : "danger-button full"} onClick={() => void toggleBan(selectedUser)} disabled={working || selectedUser.id === account.id}>{selectedUser.bannedAt ? <CheckCircle2 size={17} /> : <Ban size={17} />}{selectedUser.bannedAt ? "Restaurar acesso" : "Bloquear usuário"}</button>
          <form className="admin-notification-form" onSubmit={sendNotice}><h3><BellRing size={18} /> Enviar aviso individual</h3><Field label="Título"><input value={notification.title} onChange={(event) => setNotification({ ...notification, title: event.target.value })} /></Field><Field label="Mensagem"><textarea value={notification.body} onChange={(event) => setNotification({ ...notification, body: event.target.value })} /></Field><button className="primary-button full" type="submit" disabled={working || !notification.title.trim() || !notification.body.trim()}>Enviar aviso</button></form>
        </div>}
      </Modal>

      <Modal open={Boolean(announcement)} onClose={() => setAnnouncement(null)} eyebrow="COMUNICAÇÃO" title="Aviso do aplicativo">
        {announcement && <form className="modal-form" onSubmit={submitAnnouncement}><Field label="Título"><input value={announcement.title} onChange={(event) => setAnnouncement({ ...announcement, title: event.target.value })} /></Field><Field label="Mensagem"><textarea value={announcement.body} onChange={(event) => setAnnouncement({ ...announcement, body: event.target.value })} /></Field><Field label="Prioridade"><select value={announcement.level} onChange={(event) => setAnnouncement({ ...announcement, level: event.target.value as Announcement["level"] })}><option value="info">Informativo</option><option value="attention">Atenção</option><option value="critical">Crítico</option></select></Field><div className="setting-card compact"><div><strong>Aviso ativo</strong><small>Aparece na página inicial</small></div><Toggle checked={announcement.active} label="Aviso ativo" onChange={(active) => setAnnouncement({ ...announcement, active })} /></div><button className="primary-button full" type="submit" disabled={working || !announcement.title.trim() || !announcement.body.trim()}>Salvar aviso</button></form>}
      </Modal>

      <Modal open={Boolean(link)} onClose={() => setLink(null)} eyebrow="LINK ÚTIL" title="Link do aplicativo">
        {link && <form className="modal-form" onSubmit={submitLink}><Field label="Nome"><input value={link.label} onChange={(event) => setLink({ ...link, label: event.target.value })} placeholder="Ex.: Suporte Hydra Agro" /></Field><Field label="URL HTTPS"><input type="url" value={link.url} onChange={(event) => setLink({ ...link, url: event.target.value })} /></Field><Field label="Descrição"><textarea value={link.description || ""} onChange={(event) => setLink({ ...link, description: event.target.value })} /></Field><Field label="Posição"><input type="number" min="0" value={link.position} onChange={(event) => setLink({ ...link, position: Number(event.target.value) })} /></Field><div className="setting-card compact"><div><strong>Link ativo</strong><small>Fica visível no perfil</small></div><Toggle checked={link.active} label="Link ativo" onChange={(active) => setLink({ ...link, active })} /></div><button className="primary-button full" type="submit" disabled={working || !link.label.trim()}>Salvar link</button></form>}
      </Modal>
    </div>
  );
}
