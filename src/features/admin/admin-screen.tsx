...
import { ConfirmDialog, EmptyState, Field, Modal, ScreenHeader, Toggle } from "../../components/ui";
...
export function AdminScreen({ account, onBack }: { account: HydraAccount; onBack: () => void }) {
  ...
  async function toggleBan(user: AdminUser) {
    ...
    await run(() => setUserBan(user.id, banning, banReason), banning ? "Usuário bloqueado." : "Acesso do usuário restaurado.");
  ...
  async function confirmPendingAction() {
    ...
    if (action.kind === "ban" || action.kind === "unban") {
      await toggleBan(action.user);
      return;
    }
    if (action.kind === "role" && action.role) {
      await run(() => setUserRole(action.user.id, action.role!), "Permissão atualizada.");
      setSelectedUser(null);
      return;
    }
  ...
  async function confirmContentDelete() {
    ...
    if (!target) return;
    if (target.kind === "announcement") await run(() => deleteAnnouncement(target.item.id), "Aviso excluído.");
    else if (target.kind === "link") await run(() => deleteAppLink(target.item.id), "Link excluído.");
    else await run(() => moderatePost(target.item.id, "removed"), "Publicação removida.");
    setContentDelete(null);
  }

  async function sendNotice(event: FormEvent) {
    event.preventDefault();
    if (!selectedUser || !notification.title.trim() || !notification.body.trim()) return;
    await run(() => sendAdminNotification(selectedUser.id, notification.title, notification.body), "Aviso enviado ao usuário.");
  ...
  async function submitAnnouncement(event: FormEvent) {
    ...
    await run(() => saveAnnouncement(account.id, announcement), "Aviso publicado.");
  ...
  async function submitLink(event: FormEvent) {
    ...
    await run(() => saveAppLink(account.id, link), "Link salvo.");
    setLink(null);
  }
  ...
  return (
    <div className="screen page-enter extra-screen admin-screen">
      ...
      {loading ? (
        ...
        <section className="admin-panel-section">
          <button className="primary-button full" onClick={() => setAnnouncement(blankAnnouncement())}><Plus size={18} /> Criar aviso no aplicativo</button>
        ...
        <section className="admin-panel-section">
          <button className="primary-button full" onClick={() => setLink(blankLink())}><Plus size={18} /> Adicionar link útil</button>
      ...
      <Modal open={Boolean(selectedUser)} onClose={() => setSelectedUser(null)} eyebrow="GESTÃO DE USUÁRIO" title={selectedUser?.name || "Usuário"} wide>
        {selectedUser && <div className="admin-user-detail">
          ...
          <form className="admin-notification-form" onSubmit={sendNotice}><h3><BellRing size={18} /> Enviar aviso individual</h3><Field label="Título"><input value={notification.title} onChange={(event) => setNotification({ ...notification, title: event.target.value })} /></Field><Field label="Mensagem"><textarea value={notification.body} onChange={(event) => setNotification({ ...notification, body: event.target.value })} /></Field><div className="modal-action-row"><button className="secondary-button" type="button" onClick={() => setNotification({ title: "", body: "" })}>Cancelar aviso</button><button className="primary-button" type="submit" disabled={working || !notification.title.trim() || !notification.body.trim()}>Confirmar envio</button></div></form>
        </div>}
      </Modal>

      <Modal open={Boolean(announcement)} onClose={() => setAnnouncement(null)} eyebrow="COMUNICAÇÃO" title="Aviso do aplicativo">
        {announcement && <form className="modal-form" onSubmit={submitAnnouncement}><Field label="Título"><input value={announcement.title} onChange={(event) => setAnnouncement({ ...announcement, title: event.target.value })} /></Field><Field label="Mensagem"><textarea value={announcement.body} onChange={(event) => setAnnouncement({ ...announcement, body: event.target.value })} /></Field><Field label="Prioridade"><select value={announcement.level} onChange={(event) => setAnnouncement({ ...announcement, level: event.target.value as Announcement["level"] })}><option value="info">Informativo</option><option value="attention">Atenção</option><option value="critical">Crítico</option></select></Field><div className="setting-card compact"><div><strong>Aviso ativo</strong><small>Aparece na página inicial</small></div><Toggle checked={announcement.active} label="Aviso ativo" onChange={(active) => setAnnouncement({ ...announcement, active })} /></div><div className="modal-action-row"><button className="secondary-button" type="button" onClick={() => setAnnouncement(null)}>Cancelar</button><button className="primary-button" type="submit" disabled={working || !announcement.title.trim() || !announcement.body.trim()}>Confirmar aviso</button></div></form>}
      </Modal>

      <Modal open={Boolean(link)} onClose={() => setLink(null)} eyebrow="LINK ÚTIL" title="Link do aplicativo">
        {link && <form className="modal-form" onSubmit={submitLink}><Field label="Nome"><input value={link.label} onChange={(event) => setLink({ ...link, label: event.target.value })} placeholder="Ex.: Suporte Hydra Agro" /></Field><Field label="URL HTTPS"><input type="url" value={link.url} onChange={(event) => setLink({ ...link, url: event.target.value })} /></Field><Field label="Descrição"><textarea value={link.description || ""} onChange={(event) => setLink({ ...link, description: event.target.value })} /></Field><Field label="Posição"><input type="number" min="0" value={link.position} onChange={(event) => setLink({ ...link, position: Number(event.target.value) })} /></Field><div className="setting-card compact"><div><strong>Link ativo</strong><small>Fica visível no perfil</small></div><Toggle checked={link.active} label="Link ativo" onChange={(active) => setLink({ ...link, active })} /></div><div className="modal-action-row"><button className="secondary-button" type="button" onClick={() => setLink(null)}>Cancelar</button><button className="primary-button" type="submit" disabled={working || !link.label.trim()}>Confirmar link</button></div></form>}
      </Modal>

      <Modal open={Boolean(subscriptionUser)} onClose={() => setSubscriptionUser(null)} eyebrow="ASSINATURA" title="Gerenciar Hydra Agro+">
      ...
      <Modal open={Boolean(pendingAction)} onClose={() => setPendingAction(null)} eyebrow="CONFIRMAÇÃO OBRIGATÓRIA" title={actionCopy(pendingAction).title}>
        <div className="confirm-action"><span>{pendingAction?.kind === "activatePlus" || pendingAction?.kind === "removePlus" ? <Crown size={27} /> : <UserCog size={27} />}</span><p>{actionCopy(pendingAction).text}</p><div className="modal-action-row"><button className="secondary-button" onClick={() => setPendingAction(null)}>Cancelar</button><button className={pendingAction?.kind === "ban" || pendingAction?.kind === "removePlus" ? "danger-button" : "primary-button"} onClick={() => void confirmPendingAction()} disabled={working}>{actionCopy(pendingAction).button}</button></div></div>
      </Modal>
      <ConfirmDialog open={Boolean(contentDelete)} title={contentDelete?.kind === "announcement" ? "Excluir aviso?" : contentDelete?.kind === "link" ? "Excluir link?" : "Remover publicação?"} text={contentDelete?.kind === "announcement" ? "O aviso deixará de aparecer no aplicativo e será removido do painel." : contentDelete?.kind === "link" ? "O link deixará de aparecer no perfil de todos os usuários." : "A publicação será removida do feed pela moderação."} confirmLabel="Confirmar exclusão" busy={working} onCancel={() => setContentDelete(null)} onConfirm={() => void confirmContentDelete()} />
    </div>
  );
}
